...
static ngx_int_t
ngx_snmp_http_request_handler_for_all_server_unhealthy(ngx_http_request_t *r);
...
ngx_snmp_http_control_conf_loc_merge(ngx_conf_t *cf, void *parent, void *child){
   ...
   //     if(uscf == NULL){
   //         return "Can not got ngx_http_upstream_srv_conf_t at cf";
   //     }
   //     conf->uscf = uscf;
   // }
...
ngx_snmp_http_control_handler(ngx_http_request_t *r){
    ...
    if(unhealthy_svr_num >= scug->server_data->nelts){
        ...
        return ngx_snmp_http_request_handler_for_all_server_unhealthy(r);
    }
...
ngx_snmp_http_request_handler_for_all_server_unhealthy(ngx_http_request_t *r){
    ...
    if(shclcf->action_url.len > 0 ){
        if (ngx_strncmp(shclcf->action_url.data, "http://", sizeof("http://") - 1) == 0
            || ngx_strncmp(shclcf->action_url.data, "https://", sizeof("https://") - 1) == 0
            || ngx_strncmp(shclcf->action_url.data, "$scheme", sizeof("$scheme") - 1) == 0)
        {
            ...
            r->headers_out.location->value = shclcf->action_url;
            r->headers_out.status = NGX_HTTP_MOVED_TEMPORARILY;
            return NGX_HTTP_MOVED_TEMPORARILY;
        }
        else{
            return ngx_http_internal_redirect(r, &shclcf->action_url, &r->args);
        }
    }
    
    if(shclcf->action_msg.len > 0 ){
        r->headers_out.status = NGX_HTTP_OK;
        r->headers_out.content_length_n = shclcf->action_msg.len;
        ngx_str_set(&r->headers_out.content_type, "text/html; charset=UTF-8");
        ...
        b->last_in_chain = 1;
        b->memory = 1;
        b->pos = shclcf->action_msg.data;
        b->last = b->pos + shclcf->action_msg.len;
        out.buf = b;
        out.next = NULL;
