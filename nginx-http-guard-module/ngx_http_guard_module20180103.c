...
typedef struct {
  ...
  //  ngx_array_t                        *recv_buf;
     u_char                             *recv_buf; 
    ngx_pool_t                         *pool;
    ngx_log_t                          *log;
    snmp_oct_t                         *snmp_pdu;
   // ngx_array_t                        *snmp_pdu; 
    ngx_addr_t                         *peer;
 //   ngx_array_t                        *parseed_msg;
 //   ngx_array_t                        *result_msg;
...
static void ngx_http_guard_timer(ngx_event_t *ev){ 
   ...
   //     ngx_log_error(NGX_LOG_EMERG, ev->log, 0, "Alloc memory error"); 
   //     return;
   // }
    ...
    for(i=0;i<gmcf->guard_upstream->nelts;i++){
        ...
        for(j=0;j<gusvr.servers_snmp_data->nelts;j++){
           ...
           // snmp_para->remote_add = gsvr.server_addr.data;
            snmp_para = gsvr.snmp_cs->snmp_paras;
    ...
    if(ngx_http_guard_find_server(cf,server_addr,upstream_name) == 0){
        ...
        for (i = 0; i < gmcf->guard_upstream->nelts;i++) {
            ...
            if(ngx_strncmp(upstream_name.data,gusvr.upstream_name.data,gusvr.upstream_name.len) == 0){
                ...
                gsvr->snmp_cs->snmp_paras->port = gmcf->snmp_paras.snmp_port;
                gsvr->snmp_cs->snmp_paras->snmp_version = gmcf->snmp_paras.snmp_ver;
                gsvr->snmp_cs->snmp_paras->community = gmcf->snmp_paras.snmp_community.data;
                gsvr->snmp_cs->snmp_paras->remote_add = server_addr->data;
...
ngx_http_guard_initate_snmp_cs(ngx_http_guard_server_snmp_data_t *gsvr, ngx_conf_t *cf)
{
    ...
    gsvr->snmp_cs->obj = -1;
    if((gsvr->snmp_cs->snmp_paras = ngx_pcalloc(cf->pool, sizeof(snmp_para_t))) == NULL){
        return NGX_ERROR;
    }
    if((gsvr->snmp_cs->pc = ngx_pcalloc(cf->pool, sizeof(ngx_peer_connection_t))) == NULL){
        return NGX_ERROR;
    }
    if((gsvr->snmp_cs->recv_buf = ngx_array_create(cf->pool,8,sizeof(ngx_str_t))) == NULL){
        return NGX_ERROR;
    }
    for(i=0;i<8;i++){
        s = ngx_array_push(gsvr->snmp_cs->recv_buf);
        ss = gsvr->snmp_cs->recv_buf->elts;
        sss = &ss[i];
        if((sss->data = ngx_pcalloc(cf->pool, sizeof(u_char)*MAX_BUF_LENGTH)) == NULL){
            return NGX_ERROR;
        }
        sss->len = sizeof(u_char)*MAX_BUF_LENGTH;
    }
    gsvr->snmp_cs->pool = NULL; 
    gsvr->snmp_cs->log = NULL;
    if((gsvr->snmp_cs->snmp_pdu = ngx_array_create(cf->pool,8,sizeof(snmp_oct_t))) == NULL){
        return NGX_ERROR;
    }
    for(i=0;i<8;i++){
        snmp_pdu = (snmp_oct_t *)ngx_array_push(gsvr->snmp_cs->snmp_pdu);
        if(snmp_pdu == NULL){
            return NGX_ERROR;
        }
    }
 //   if((gsvr->snmp_cs->snmp_pdu = ngx_pcalloc(cf->pool, sizeof(snmp_oct_t))) == NULL){
 //       return NGX_ERROR;
 //   }
    if((gsvr->snmp_cs->peer = ngx_pcalloc(cf->pool, sizeof(ngx_addr_t))) == NULL){
        return NGX_ERROR;
    }
  //  gsvr->snmp_cs->peer = NULL;
    if((gsvr->snmp_cs->parseed_msg = ngx_array_create(cf->pool,8,sizeof(receive_msg_t))) == NULL){
        return NGX_ERROR;
    }
    for(i=0;i<8;i++){
        parseed_msg = ngx_array_push(gsvr->snmp_cs->parseed_msg);
        if(parseed_msg == NULL){
            return NGX_ERROR;
        }
    }
 //   if((gsvr->snmp_cs->parseed_msg = ngx_pcalloc(cf->pool, sizeof(receive_msg_t))) == NULL){
  //      return NGX_ERROR;
  //  }
    if((gsvr->snmp_cs->result_msg = ngx_array_create(cf->pool,8,sizeof(snmp_msg_t))) == NULL){
        return NGX_ERROR;
    }
    for(i=0;i<8;i++){
        result_msg = ngx_array_push(gsvr->snmp_cs->result_msg);
        if(result_msg == NULL){
            return NGX_ERROR;
        }
    }
    if((gsvr->snmp_cs->obj = ngx_array_create(cf->pool,8,sizeof(ngx_int_t))) == NULL){
        return NGX_ERROR;
    }
    for(i=0;i<8;i++){
        objs = ngx_array_push(gsvr->snmp_cs->obj);
        if(objs == NULL){
            return NGX_ERROR;
        }
        objs = gsvr->snmp_cs->obj.elts;
        objs[i] = 0;
    }
    //if((gsvr->snmp_cs->result_msg = ngx_pcalloc(cf->pool, sizeof(snmp_msg_t))) == NULL){
    //    return NGX_ERROR;
   // }
    gsvr->snmp_cs->timeout = NGX_ETIMEDOUT;
    gsvr->snmp_cs->status = -1;
...
ngx_http_guard_send(ngx_event_t *wev)
{
   // ngx_peer_connection_t                     *pc;
    ngx_connection_t                          *c;
    ngx_http_guard_connect_session_t          *snmp_cs;
    snmp_oct_t                                *snmp_pdus,snmp_pdu;
    ngx_event_t                               *rev;
    
    c = wev->data;
    snmp_cs = c->data;
    snmp_pdus = snmp_cs->snmp_pdu->elts;
    snmp_pdu = snmp_pdus[snmp_cs->obj];
    if (c->destroyed) 
    {
...
ngx_http_guard_recv(ngx_event_t *rev)
{
    ...
    ngx_str_t                                 *ss, *s;
    u_char                                    *recv_buf;
    receive_msg_t                             *parseed_msgs,*parseed_msg; 
    snmp_msg_t                                *result_msgs,*result_msg; 
    ...
    ss =  snmp_cs->recv_buf->elts;
    s = &ss[snmp_cs->obj];
    recv_buf = s->data;
    ngx_memzero(recv_buf, s->len);
    while(rev->ready){
    ...
    parseed_msgs = snmp_cs->parseed_msg->elts;
    parseed_msg = &parseed_msgs[snmp_cs->obj];
    ...
    result_msgs = snmp_cs->result_msg->elts;
    result_msg = &result_msgs[snmp_cs->obj];
    ...
    if(result_msg->type == 2){
        ngx_log_error(NGX_LOG_EMERG, rev->log, 0, 
                "We get a strig SNMP message:%s of %d",result_msg->valuemsg,snmp_cs->obj);
    }
    else{
        ngx_log_error(NGX_LOG_EMERG, rev->log, 0, 
                
                "We get a numberic SNMP message:%d of %d",result_msg->value,snmp_cs->obj);
    }
...
ngx_http_guard_handler_snmp_data(ngx_http_guard_connect_session_t *snmp_cs)
{
    ...
    result_msg = snmp_cs->result_msg->elts;
    snmp_data = snmp_cs->snmp_data;
    switch(snmp_cs->obj){
        case 0:
            if(result_msg[snmp_cs->obj].type != 2){
            ngx_log_error(NGX_LOG_EMERG,snmp_cs->log, 0, 
                "guard: get a unavailable cpu load value");
            }
            snmp_data->cpu_load = atof((char *)result_msg[snmp_cs->obj].valuemsg);
            break;
        case 1:
           if(result_msg[snmp_cs->obj].type != 1){
                ngx_log_error(NGX_LOG_EMERG,snmp_cs->log, 0, 
