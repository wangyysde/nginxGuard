...
static ngx_int_t 
ngx_http_guard_get_snmp_data(snmp_para_t *snmp_para, ngx_event_t *ev,ngx_http_guard_server_snmp_data_t gsvr,ngx_str_t upstream_name,char *obj_name);
...
ngx_http_guard_parse_snmp_paras(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
static ngx_int_t 
ngx_http_guard_create_connection(ngx_http_guard_connect_session_t *snmp_cs,ngx_event_t *wev);
static ngx_int_t
...
static void
ngx_http_guard_send(ngx_event_t *wev);
static void
ngx_http_guard_recv(ngx_event_t *rev);
static ngx_int_t
ngx_http_guard_build_snmp_send_pdu(ngx_http_guard_connect_session_t *snmp_cs);
ngx_int_t 
ngx_http_guard_init_snmp_cs(ngx_http_guard_connect_session_t  **snmp_cs,ngx_event_t *ev);
ngx_int_t 
ngx_http_guard_destroy_snmp_cs(ngx_http_guard_connect_session_t  *snmp_cs);
static void
ngx_http_guard_close(ngx_connection_t *c);
static ngx_int_t
ngx_http_guard_handler_snmp_data(ngx_http_guard_connect_session_t *snmp_cs);
...
static void ngx_http_guard_timer(ngx_event_t *wev){ 
    ...
    ngx_add_timer(wev,gmcf->snmp_paras.get_interval*1000);
    ngx_log_error(NGX_LOG_EMERG, wev->log, 0, "This is running"); 
    if((snmp_para = (snmp_para_t *)malloc(sizeof(snmp_para_t))) == NULL){
    ...
    for(i=0;i<gmcf->guard_upstream->nelts;i++){
        ...
        for(j=0;j<gusvr.servers_snmp_data->nelts;j++){
            ...
            if(ngx_http_guard_get_snmp_data(snmp_para,wev,gsvr,gusvr.upstream_name,"cpu_load")!= NGX_OK){
                continue;
            }
            snmp_para->oid = gmcf->snmp_paras.memused_oid.data;
            if(ngx_http_guard_get_snmp_data(snmp_para,wev,gsvr,gusvr.upstream_name,"mem_used")!= NGX_OK){
                continue;
            }
            snmp_para->oid =  gmcf->snmp_paras.memtotal_oid.data;
            if(ngx_http_guard_get_snmp_data(snmp_para,wev,gsvr,gusvr.upstream_name,"mem_total")!= NGX_OK){
                continue;
            }
            snmp_para->oid = gmcf->snmp_paras.memssc_oid.data;
            if(ngx_http_guard_get_snmp_data(snmp_para,wev,gsvr,gusvr.upstream_name,"mem_sscontext")!= NGX_OK){
                continue;
            }
            snmp_para->oid = gmcf->snmp_paras.nicthroughput_oid.data;
            if(ngx_http_guard_get_snmp_data(snmp_para,wev,gsvr,gusvr.upstream_name,"interface_current_byes")!= NGX_OK){
                continue;
            }
           // snmp_para->oid = gmcf->snmp_paras.iorate_oid.data;
         //   if(ngx_http_guard_get_snmp_data(snmp_para,wev,gsvr,gusvr.upstream_name,"disk_io_rate")!= NGX_OK){
       //         continue;
        //    }
        }
    }
...
ngx_int_t ngx_http_guard_init_process (ngx_cycle_t *cycle){
    ...
    gdrive_timer_wev.handler=ngx_http_guard_timer;
...
ngx_http_guard_parse_snmp_paras(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if(ngx_strncmp(cmd->name.data,"guard_snmpport",ngx_strlen("guard_snmpport")) == 0){
        ...
        gmcf->snmp_paras.nicthroughput_oid = value[1]; 
        return NGX_CONF_OK;
    }
    else if(ngx_strncmp(cmd->name.data,"guard_iorate",ngx_strlen("guard_iorate")) == 0){
        gmcf->snmp_paras.iorate_oid = value[1]; 
        return NGX_CONF_OK;
    }
    else{
...
static ngx_int_t 
ngx_http_guard_create_connection(ngx_http_guard_connect_session_t *snmp_cs,ngx_event_t *ev)
{
    ngx_peer_connection_t                     *pc;
    ngx_int_t                                  rc;
    ngx_connection_t                          *c;
   ...
   // pc->sockaddr = (struct sockaddr *)server_addr;
    ...
    c->pool = snmp_cs->pool;
    ev = c->write;
    ngx_add_timer(ev,snmp_cs->timeout);
    c->read->handler = ngx_http_guard_recv;
    c->write->handler = ngx_http_guard_dummy_send;
    snmp_cs->connection = c;
    ngx_http_guard_send(c->write);
    return NGX_OK;
...
ngx_http_guard_send(ngx_event_t *wev)
{
   // ngx_peer_connection_t                     *pc;
    ...
    if(rev->timer_set){
        ngx_del_timer(rev);
        ngx_add_timer(c->read,snmp_cs->timeout);
    }
    else{
        ngx_add_timer(c->read,snmp_cs->timeout);
    }
...
ngx_http_guard_recv(ngx_event_t *rev)
{
    ...
    while(rev->ready){
        n = c->recv(c,snmp_cs->recv_buf,snmp_cs->bufsize);
        if(n == NGX_AGAIN){
            break;
        }
        if(n == NGX_ERROR){
    ...
    if(!initate_snmp_receive_msg(snmp_cs->parseed_msg)){
        ngx_log_error(NGX_LOG_EMERG, rev->log, NGX_ETIMEDOUT, 
                "guard: initate snmp receive msg error");
        goto end; 
    }
    if((snmp_err_code=snmp_parase_response(snmp_cs->recv_buf,snmp_cs->parseed_msg)) != 0){
        ngx_log_error(NGX_LOG_EMERG, rev->log, NGX_ETIMEDOUT, 
                "guard: get a unavailable SNMP message:%s",snmp_strerror(snmp_err_code));
        goto end; 
    }
    if((snmp_err_code=snmp_get_response_msg(snmp_cs->parseed_msg,snmp_cs->result_msg))!=0){
        ngx_log_error(NGX_LOG_EMERG, rev->log, 0, 
                "guard: get the SNMP message error:%s",snmp_strerror(snmp_err_code));
        goto end; 
    }
    if(snmp_cs->result_msg->type == 2){
        ngx_log_error(NGX_LOG_EMERG, rev->log, 0, 
                "We get a strig SNMP message:%s",snmp_cs->result_msg->valuemsg);
    }
    else{
        ngx_log_error(NGX_LOG_EMERG, rev->log, 0, 
                "We get a numberic SNMP message:%d",snmp_cs->result_msg->value);
    }
    snmp_cs->status = NGX_OK;
    ngx_close_connection(rev->data);
    ngx_http_guard_handler_snmp_data(snmp_cs);
  //  ngx_http_guard_destroy_snmp_cs(snmp_cs);
    return;
   
end:
    ngx_http_guard_destroy_snmp_cs(snmp_cs);
    //snmp_cs->status = NGX_ERROR;
    return;
}
...
ngx_http_guard_init_snmp_cs(ngx_http_guard_connect_session_t  **snmp_cs,ngx_event_t *ev)
{
    ngx_pool_t                                *pool;
    ngx_peer_connection_t                     *pc;
    ngx_http_guard_connect_session_t          *tmp_snmp_cs;
    ngx_addr_t                                *peer;
    
    *snmp_cs = NULL;
    pool = NULL;
    if((tmp_snmp_cs = (ngx_http_guard_connect_session_t *)malloc(sizeof(ngx_http_guard_connect_session_t))) == NULL)
    {
        ngx_log_error(NGX_LOG_EMERG, ev->log, 0, "Alloc memory for connect session error");
        return NGX_ERROR; 
    }
    if((tmp_snmp_cs->parseed_msg = malloc(sizeof(receive_msg_t))) == NULL){
        ngx_log_error(NGX_LOG_EMERG, ev->log, 0, "Alloc memory for parseed msg  error");
        goto free_snmp_cs;
    }
    if((tmp_snmp_cs->result_msg = malloc(sizeof(snmp_msg_t))) == NULL){
        ngx_log_error(NGX_LOG_EMERG, ev->log, 0, "Alloc memory for result msg  error");
        goto free_parseed_msg;
    }
    *snmp_cs = tmp_snmp_cs;
    tmp_snmp_cs->log = ev->log; 
    pool = ngx_create_pool(4096, ev->log);
    if (pool == NULL) {
        ngx_log_error(NGX_LOG_EMERG, ev->log, 0, "Create connection pool error");
        goto free_result_msg;
    }
    tmp_snmp_cs->pool = pool;
    peer = ngx_pcalloc(pool, sizeof(ngx_addr_t));
    if (peer == NULL) {
         ngx_log_error(NGX_LOG_EMERG,ev->log, 0,"Alloc memory  for peer error");
         goto free_pool;
    }
    tmp_snmp_cs->peer = peer; 
    pc = ngx_pcalloc(tmp_snmp_cs->pool, sizeof(ngx_peer_connection_t));
    if(pc == NULL)
    {
        ngx_log_error(NGX_LOG_EMERG, ev->log, 0, "Create connection pool error");
        goto free_peer;
    ...
    tmp_snmp_cs->pc =  pc; 
    tmp_snmp_cs->bufsize = sizeof(char)*MAX_BUF_LENGTH;
    if((tmp_snmp_cs->recv_buf = malloc(tmp_snmp_cs->bufsize)) == NULL){
        ngx_log_error(NGX_LOG_EMERG, ev->log, 0, "Alloc memory for recv buf error");
        goto free_pc;
    }
    if((tmp_snmp_cs->snmp_pdu = malloc(sizeof(snmp_oct_t))) == NULL)
    {
        ngx_log_error(NGX_LOG_EMERG, ev->log, 0, "Alloc memory for snmp pdu error");
        goto free_recv_buf;
    }
    tmp_snmp_cs->timeout = NGX_ETIMEDOUT;
    return NGX_OK;
    
    
    free_recv_buf:
        free(tmp_snmp_cs->recv_buf);
        tmp_snmp_cs->recv_buf = NULL;
    free_pc:
        ngx_pfree(tmp_snmp_cs->pool,tmp_snmp_cs->pc);
    free_peer:
        ngx_pfree(tmp_snmp_cs->pool,peer);
    free_pool:
        ngx_destroy_pool(tmp_snmp_cs->pool);
    free_result_msg:
        free(tmp_snmp_cs->result_msg);
    free_parseed_msg:
        free(tmp_snmp_cs->parseed_msg);
    free_snmp_cs:
        free(tmp_snmp_cs);
        tmp_snmp_cs = NULL;
    return NGX_ERROR;
...
ngx_http_guard_get_snmp_data(snmp_para_t *snmp_para, ngx_event_t *ev,ngx_http_guard_server_snmp_data_t gsvr,ngx_str_t upstream_name, char *obj_name){
    ...
    if(ngx_http_guard_init_snmp_cs(&snmp_cs,ev) != NGX_OK)
    {
        return NGX_ERROR;
    }
    rc = ngx_parse_addr(snmp_cs->pool, snmp_cs->peer,
            gsvr.server_addr.data, gsvr.server_addr.len);
    snmp_cs->snmp_data = &gsvr.snmp_data;
    ngx_str_set(&snmp_cs->obj_name,obj_name);
    switch (rc){
        case NGX_OK:
            break;
        case NGX_DECLINED:
    ...
    snmp_cs->peer->name = upstream_name;
    snmp_cs->snmp_paras = snmp_para;
    snmp_cs->upstream_name = upstream_name;
    if(ngx_http_guard_build_snmp_send_pdu(snmp_cs) != NGX_OK ){
        ngx_http_guard_destroy_snmp_cs(snmp_cs);
            return NGX_ERROR;
        }
    if(ngx_http_guard_create_connection(snmp_cs,ev ) != NGX_OK){
        ngx_http_guard_destroy_snmp_cs(snmp_cs);
        return NGX_ERROR;
    }
    return NGX_OK;
...
ngx_http_guard_handler_snmp_data(ngx_http_guard_connect_session_t *snmp_cs)
{
    ...
    if(snmp_cs->status != NGX_OK){
        ngx_log_error(NGX_LOG_EMERG,snmp_cs->log, 0, 
                "guard: get a unavailable SNMP message");
        return NGX_ERROR;
    }
    result_msg = snmp_cs->result_msg;
    snmp_data = snmp_cs->snmp_data;
    if(ngx_strncmp(snmp_cs->obj_name.data,"cpu_load",snmp_cs->obj_name.len) == 0){
    ...
    else if(ngx_strncmp(snmp_cs->obj_name.data,"mem_used",snmp_cs->obj_name.len) == 0){
    ...
    else if(ngx_strncmp(snmp_cs->obj_name.data,"mem_total",snmp_cs->obj_name.len) == 0){
        if(result_msg->type != 1){
            ngx_log_error(NGX_LOG_EMERG,snmp_cs->log, 0, 
    ...
    else if(ngx_strncmp(snmp_cs->obj_name.data,"mem_sscontext",snmp_cs->obj_name.len) == 0){
        if(result_msg->type != 1){
            ngx_log_error(NGX_LOG_EMERG,snmp_cs->log, 0, 
    ...
    else if(ngx_strncmp(snmp_cs->obj_name.data,"interface_current_byes",snmp_cs->obj_name.len) == 0){
        if(result_msg->type != 1){
            ngx_log_error(NGX_LOG_EMERG,snmp_cs->log, 0, 
                "guard: get a unavailable interface current byes value");
        }
        snmp_data->interface_last_byes = snmp_data->interface_current_byes;
        snmp_data->interface_current_byes = result_msg->value;
    }
    else if(ngx_strncmp(snmp_cs->obj_name.data,"disk_io_rate",snmp_cs->obj_name.len) == 0){
        if(result_msg->type != 2){
            ngx_log_error(NGX_LOG_EMERG,snmp_cs->log, 0, 
                "guard: get a unavailable disk io rate value");
        }
        snmp_data->disk_io_rate = atof((char *)result_msg->valuemsg);
    }
    else{
        ngx_log_error(NGX_LOG_EMERG,snmp_cs->log, 0, 
