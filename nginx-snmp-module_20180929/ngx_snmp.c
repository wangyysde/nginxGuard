...
ngx_snmp_init_process(ngx_cycle_t *cycle)
{
    ...
    ngx_snmp_item_value_t                   **item_values,*item_value;
    ...
    for(i=0;i<scmcf->upstream_group.nelts;i++){
        ...
        for(j=0;j<scug.server_data->nelts;j++){
            scsd = scsds[j];
            scsds[j].log = cycle->log;
            item_values = scsd.item_value->elts;
            for(k=0;k<scsd.item_value->nelts;k++){
                item_value = item_values[k];
                if(ngx_snmp_initate_object_session(cycle,item_value->item,item_value) != NGX_OK){
                    return NGX_ERROR;
                } 
            }
            if(ngx_snmp_initate_timer_for_server(&scsds[j]) != NGX_OK){
                return NGX_ERROR; 
            }
        }
    }
