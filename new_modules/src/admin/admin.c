...
static ngx_int_t 
ngx_http_bzhy_admin_handle_variables(ngx_http_request_t *r);
static ngx_int_t ngx_http_bzhy_admin_get_variables(ngx_http_request_t *r,
    ngx_int_t var_method,ngx_str_t *name,ngx_str_t **value);
...
ngx_http_bzhy_admin_init(ngx_http_request_t *r)
{
    ...
    rc = ngx_http_bzhy_admin_handle_variables(r);
    if(rc != NGX_OK){
        ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
        return;
    }
...
ngx_http_bzhy_admin_handle_variables(ngx_http_request_t *r){
    ngx_http_bzhy_admin_ctx_t            *bzhy_admin_ctx;
    ngx_int_t                            len,namelen,valuelen,rc;
    ngx_str_t                            args,*sessionid,*sessionname;
    u_char                               *var,*name,*value,*pos,*buf,*p,*last,*start,*end;
    bzhy_admin_http_variable_t           *variable;
    ngx_buf_t                            *b;
    ngx_chain_t                          *cl;
    ngx_array_t                          *cookies;
    ngx_table_elt_t                      **elements;
    ngx_uint_t                          i;
    ...
    elements = cookies->elts;
    for (i = 0; i < cookies->nelts; i++) {
        ngx_log_debug2(NGX_LOG_DEBUG_HTTP, cookies->pool->log, 0,
            "Get cookie: \"%V: %V\"", &elements[i]->key, &elements[i]->value);
        
        start = elements[i]->value.data;
        end = elements[i]->value.data + elements[i]->value.len;
        
        pos = (u_char *)ngx_strchr(start,'=');
        if(pos == NULL){
            continue;
        }
        value = pos;
        len = pos - start;
        for(last = --pos; last>start && *last == ' ';last--){
            len--;
        }
        
        variable = ngx_array_push( &bzhy_admin_ctx->cookie_variables);
        if(variable == NULL){
           return NGX_ERROR;
        }
        
        variable->name.data = start;
        variable->name.len = len;
        
        for(++value;value<end && *value = ' ';value++){
             /* void */
        }
        
        for (last = value; last < end && *last != ';'; last++) {
            /* void */
        }
        
        variable->value.data = value;
        variable->value.len = last - value;
    }
    
    sessionid->len = 0;
    ngx_str_set(sessionname,BZHY_ADMIN_SESSION_ID);
    rc = ngx_http_bzhy_admin_get_variables(r,NGX_HTTP_BZHY_ADMIN_VAR_METHOD_COOKIE,sessionname,&sessionid);
    if(rc == NGX_ERROR){
        return NGX_ERROR;
    }
    if(rc == NGX_OK){
        rc = ngx_http_bzhy_admin_get_variables(r,NGX_HTTP_BZHY_ADMIN_VAR_METHOD_GET,sessionname,&sessionid);
    }
    if(rc == NGX_ERROR){
        return NGX_ERROR;
    }
    if(rc == NGX_DONE){
        return ngx_http_bzhy_admin_handle_session_variable(r,sessionid);
    }
...
static ngx_int_t
ngx_http_bzhy_admin_get_variables(ngx_http_request_t *r,ngx_int_t var_method,ngx_str_t *name,ngx_str_t **value){
    ngx_http_bzhy_admin_ctx_t            *bzhy_admin_ctx;
    ngx_array_t                          *var_array;
    bzhy_admin_http_variable_t           *vars,var;
    bzhy_admin_http_variable_method_e    method;
    ngx_int_t                            rc,found;
    ngx_uint_t                           i;
    ...
    switch(var_method){
        case NGX_HTTP_BZHY_ADMIN_VAR_METHOD_GET:
            var_array = &bzhy_admin_ctx->get_variables;
            break;
        case NGX_HTTP_BZHY_ADMIN_VAR_METHOD_POST:
            var_array = &bzhy_admin_ctx->post_variables;
            break;
        case NGX_HTTP_BZHY_ADMIN_VAR_METHOD_COOKIE:
            var_array = &bzhy_admin_ctx->cookie_variables;
            break;
        case NGX_HTTP_BZHY_ADMIN_VAR_METHOD_SESSION:
            var_array = &bzhy_admin_ctx->session_variables;
            break;
        case NGX_HTTP_BZHY_ADMIN_VAR_METHOD_ALL:
            for(method=NGX_HTTP_BZHY_ADMIN_VAR_METHOD_GET;method<NGX_HTTP_BZHY_ADMIN_VAR_METHOD_ALL;method++){
                rc = ngx_http_bzhy_admin_get_variables(r,method,name,value);
    ...
    if(var_method != NGX_HTTP_BZHY_ADMIN_VAR_METHOD_ALL){
        vars = var_array->elts;
        found = 0;
        for(i=0;i<var_array->nelts;i++){
            var = vars[i];
            if(name->len == var.name.len && ngx_strncasecmp(name->data,var.name.data,name->len) == 0){
                *value->data = var.value.data;
                *value->len = var.value.len;
                found = 1;
            }
        }
...
static ngx_int_t
ngx_http_bzhy_admin_handle_session_variable(ngx_http_request_t *r,ngx_str_t *sessionid){
    ngx_http_bzhy_admin_ctx_t            *bzhy_admin_ctx;
    admin_core_svr_conf_t                *acscf;
    
    bzhy_admin_ctx = ngx_http_get_module_ctx(r, admin_core_module);
    acscf = ngx_http_get_module_srv_conf(r,admin_core_module);
    if(bzhy_admin_ctx ==  NULL || acscf == NULL){
        return NGX_ERROR;
    }
